...
class PointPillarPyramidLossPrm(PointPillarDepthLoss):
    def __init__(self, args):
        ...
        self.single_det_weight = args.get('single_det_weight', 0.8)
        self.single_occ_weight = args.get('single_occ_weight', 0.2)
    ...
    def forward_collab(self, output_dict, target_dict, suffix):
        ...
        single_det_loss = super().forward(output_dict, target_dict, suffix)
        occ_loss = single_det_loss.new_tensor(0.0)

        if self.single_occ_weight > 0 and 'occ_single_list' in output_dict:
            batch_size = target_dict['pos_equal_one'].shape[0]
        ...
        total_loss = self.single_det_weight * single_det_loss + self.single_occ_weight * occ_loss
        self.loss_dict = {
            'single_det_loss': single_det_loss.item(),
            'pyramid_loss': occ_loss.item() if torch.is_tensor(occ_loss) else float(occ_loss),
            'total_loss': total_loss.item()
        }
    ...
    def logging(self, epoch, batch_id, batch_len, writer = None, suffix=""):
        ...
        single_det_loss = self.loss_dict.get('single_det_loss', 0)


        print("[epoch %d][%d/%d]%s || Loss: %.4f || Conf Loss: %.4f"
              " || Loc Loss: %.4f || Dir Loss: %.4f || IoU Loss: %.4f || Depth Loss: %.4f || Pyramid Loss: %.4f || Single Det Loss: %.4f" % (
                  epoch, batch_id + 1, batch_len, suffix,
                  total_loss, cls_loss, reg_loss, dir_loss, iou_loss, depth_loss, pyramid_loss, single_det_loss))
        ...
        if not writer is None:
            ...
            writer.add_scalar('Pyramid_loss' + suffix, pyramid_loss,
                epoch*batch_len + batch_id)
            writer.add_scalar('Single_Det_loss' + suffix, single_det_loss,
                epoch*batch_len + batch_id)
