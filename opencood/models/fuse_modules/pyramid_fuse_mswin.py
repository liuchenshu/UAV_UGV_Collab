...
from opencood.models.sub_modules.mswin import PyramidWindowAttention

def weighted_fuse(x, score, record_len, affine_matrix, align_corners,window_attn=None):
    """
    Parameters
    ----------
    x : torch.Tensor
    ...
    for b in range(B):
        ...
        scores_in_ego = warp_affine_simple(split_score[b],
                                           t_matrix[i, :, :, :],
                                           (H, W), align_corners=align_corners)
        # ★ 注入点：在加权求和前用窗口注意力做跨 agent 特征精炼
        if window_attn is not None:
            # PyramidWindowAttention 期望输入 (B, L, H, W, C)
            feat = feature_in_ego.permute(0, 2, 3, 1).unsqueeze(0)  # (1, N, H, W, C)
            feat = window_attn(feat)                                  # (1, N, H, W, C)
            feature_in_ego = feat.squeeze(0).permute(0, 3, 1, 2)     # (N, C, H, W)
...
class PyramidFusion(ResNetBEVBackbone):
    def __init__(self, model_cfg, input_channels=64):
        ...
        win_cfg = model_cfg.get('window_attn', {})
        self.use_window_attn = win_cfg.get('enabled', False)

        if self.use_window_attn:
            self.window_attns = nn.ModuleList()
            fuse_methods = win_cfg.get('fuse_method', ['naive'] * self.num_levels)
            
            # 兼容写法：传字符串则所有level共用，传列表则逐level指定
            if isinstance(fuse_methods, str):
                fuse_methods = [fuse_methods] * self.num_levels
            for i in range(self.num_levels):
                self.window_attns.append(
                    PyramidWindowAttention(
                        dim=self.model_cfg['num_filters'][i],
                        heads=win_cfg.get('heads', [2, 4]),
                        dim_heads=win_cfg.get('dim_heads', [32, 32]),
                        drop_out=win_cfg.get('dropout', 0.0),
                        window_size=win_cfg.get('window_size', [4, 8]),
                        relative_pos_embedding=win_cfg.get('relative_pos_embedding', True),
                        # fuse_method=win_cfg.get('fuse_method', 'naive'),
                        fuse_method=fuse_methods[i],   # ← 每个level单独传
                    )
                )
    ...
    def forward_collab(self, spatial_features, record_len, affine_matrix, agent_modality_list = None, cam_crop_info = None):
        ...
        for i in range(self.num_levels):
            ...
            win_attn_i = self.window_attns[i] if self.use_window_attn else None
            fused_feature_list.append(
                weighted_fuse(
                    feature_list[i], score, record_len,
                    affine_matrix, self.align_corners,
                    window_attn=win_attn_i,   # ← 新增
                )
            )
