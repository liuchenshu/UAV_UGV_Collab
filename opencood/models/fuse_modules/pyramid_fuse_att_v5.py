...
class PositionEmbeddingSine2D(nn.Module):
    ...
    def forward(self, tensor_list):
        ...
        # 拼接 y 和 x 的位置编码，最终通道数为 num_pos_feats * 2
...
class MSDeformAttnFuse(nn.Module):
    ...
    def __init__(self, d_model=256, n_heads=4, n_points=4):
        ...
        self.sampling_offsets = nn.Linear(
            d_model*2, n_heads * self.n_levels * n_points * 2
        )
        # 注意力权重预测
        self.attention_weights = nn.Linear(
            d_model*2, n_heads * self.n_levels * n_points
        )
    ...
    def forward(self, ego_feat, collab_feat):
        ...
        pos = self.pos_embed(ego_feat) # [B, C, H, W]
        query_ego_with_pos = (ego_feat + pos).flatten(2).transpose(1, 2)       # [B, HW, C]
        value_collab_with_pos = (collab_feat + pos).flatten(2).transpose(1, 2) # [B, HW, C]

        
        query_concat = torch.cat([query_ego_with_pos, value_collab_with_pos], dim=-1) # [B, HW, 2C]
        # Value 投影 + 分头
        ...
        # 预测偏移 + 注意力权重
        sampling_offsets = self.sampling_offsets(query_concat).view(
...
def ms_deform_attn_fuse(x, score, record_len, affine_matrix, align_corners, fuse_module):
    ...
    for b in range(B):
        ...
        aligned_feats = aligned_feats * scores_in_ego
