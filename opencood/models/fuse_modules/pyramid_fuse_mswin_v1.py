...
class PyramidFusion(ResNetBEVBackbone):
    def __init__(self, model_cfg, input_channels=64):
        ...
        if self.use_window_attn:
            self.norms = nn.ModuleList([
                    nn.LayerNorm(self.model_cfg['num_filters'][i])
                    for i in range(self.num_levels)
                ])
            self.post_window_attns = nn.ModuleList()   

            if isinstance(win_cfg.get('fuse_method', 'naive'), str):
                fuse_methods = [win_cfg.get('fuse_method', 'naive')] * self.num_levels
            else:
                fuse_methods = win_cfg['fuse_method']

            for i in range(self.num_levels):
                self.post_window_attns.append(
    ...
    def forward_collab(self, spatial_features, record_len, affine_matrix, agent_modality_list = None, cam_crop_info = None):
        ...
        for i in range(self.num_levels):
            ...
            fused=weighted_fuse(feature_list[i], score, record_len, affine_matrix, self.align_corners)
            # === ⭐ MSwin 在 fusion 后 ⭐ ===
            if self.use_window_attn:
                B, C, H, W = fused.shape

                # → (B, 1, H, W, C)
                feat = fused.permute(0, 2, 3, 1).unsqueeze(1)

                # MSwin
                feat_out = self.post_window_attns[i](feat)

                # → (B, C, H, W)
                feat_out = feat_out.squeeze(1).permute(0, 3, 1, 2)

                feat_out = self.norms[i](feat_out.permute(0,2,3,1)).permute(0,3,1,2)

                # === ⭐ residual（非常重要）===
                fused = fused + feat_out*0.8 # 这里的权重可以调整，或者直接相加（不加权重）
